...
class DatasetSynthesisModule:
    def __init__(self, knowledge_links, prompt_file, llm_name="groq", embedding_model_name="all-MiniLM-L12-v2", api_key=None, num_entries=5000):
...
if __name__ == "__main__":
    ...
    num_entries = 5000 #define total entries to generate
    ...
    for i in range(num_batches): #loop through batches
        print(f"Generating batch {i+1}/{num_batches}") # Print the batch progress
        dataset = synthesis_module.generate_dataset(num_examples=batch_size) #run synthesis module

        for item in dataset: #think tags removed
            item["text2"] = item["text2"].replace("<think>", "").replace("</think>", "").strip() #think tags removed

        final_dataset.extend(dataset) #add the data to total data

        print(f"Total records generated: {len(final_dataset)}")

        if len(final_dataset) >= num_entries: #stop the loop if the number of records generated is greater than 5000
            break
    
    synthesis_module.save_dataset(final_dataset, "output_5000.json")
